function varargout = interfaceCSSfinal(varargin)
% INTERFACECSSFINAL MATLAB code for interfaceCSSfinal.fig
%      INTERFACECSSFINAL, by itself, creates a new INTERFACECSSFINAL or raises the existing
%      singleton*.
%
%      H = INTERFACECSSFINAL returns the handle to a new INTERFACECSSFINAL or the handle to
%      the existing singleton*.
%
%      INTERFACECSSFINAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INTERFACECSSFINAL.M with the given input arguments.
%
%      INTERFACECSSFINAL('Property','Value',...) creates a new INTERFACECSSFINAL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before interfaceCSSfinal_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to interfaceCSSfinal_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help interfaceCSSfinal

% Last Modified by GUIDE v2.5 21-Jun-2022 16:23:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @interfaceCSSfinal_OpeningFcn, ...
                   'gui_OutputFcn',  @interfaceCSSfinal_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before interfaceCSSfinal is made visible.
function interfaceCSSfinal_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to interfaceCSSfinal (see VARARGIN)

% Choose default command line output for interfaceCSSfinal
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes interfaceCSSfinal wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = interfaceCSSfinal_OutputFcn(hObject, eventdata, handles) 
...
set(handles.mostrar,'String',num2str(K))
    ...
    set(handles.mostrar,'string','No se pueden obtener las ganancias porque el sistema no es controlable');
end
...
if  (h ~= 0)
    ...
    set(handles.mostrar,'string','No se pueden obtener las ganancias porque el sistema no es observable');
end
